...
pub struct QuartzPlugin;

impl Plugin for QuartzPlugin {
    fn build(&self, app: &mut App) {
        app.add_plugins(PanCamPlugin)
            ...
            .insert_resource(ClearColor(Color::hsla(0.0, 0.0, 0.0, 1.0)))
            .insert_resource(DefaultDrawColor(Hsla::new(270.0, 1.0, 0.5, 1.0)))
            .insert_resource(DefaultDrawVerts(4))
            .insert_resource(HighlightColor(Hsla::new(0.0, 1.0, 0.5, 1.0)))
            .insert_resource(ConnectionColor(Hsla::new(0.0, 1.0, 1.0, 0.7)))
            .insert_resource(ConnectionWidth(4.0))
            .insert_resource(CommandColor(Hsla::new(0.0, 0.0, 0.7, 1.0)))
            .insert_resource(IndicatorColor(Hsla::new(0.0, 1.0, 0.5, 0.3)))
            .insert_resource(TextSize(0.1))
            .insert_resource(NodeLimit(500))
            .insert_resource(
                Version(format!("{} {}", env!("CARGO_PKG_VERSION"), env!("COMMIT_HASH")))
            )
            ...
            .add_systems(
                Update,
                update_selection.after(update_cursor_info).run_if(in_state(Mode::Edit))
            )
            ...
            .add_systems(
                Update,
                update_vertices.after(update_selection).run_if(in_state(Mode::Edit))
            )
            ...
            .add_systems(
                Update,
                rotate_selected.after(update_selection).run_if(in_state(Mode::Edit))
            )
            ...
            .add_systems(
                PostUpdate,
                sort_by_order.before(process).run_if(on_event::<OrderChange>())
            )
